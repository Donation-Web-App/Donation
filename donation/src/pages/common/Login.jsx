...
export function Login () {
	...
	async function handleLogin(e) {
		...
		try {
			...
			if (response.data.status == "success") {
				...
				const { token, user : { role, firstname } } = response.data.data;
	...
	return (
		<Page>
			<form
				className='w-formWidth mx-auto'
				onSubmit={handleLogin}
			>
				<h1 className='text-3xl text-center'>
					Welcome to{" "}
					<span className='text-primary font-bold'>CACSA-UI</span>	
				</h1>
				<TextInput
					type='email'
				...
				<div className='flex items-center justify-between'>
					<div className='flex items-center gap-2'>
						<input type='checkbox' onClick={() => setShowPassword((current) => !current)} />
